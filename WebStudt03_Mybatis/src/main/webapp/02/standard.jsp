...
<pre>
	: 서블릿의 파생 스펙으로 템플릿 엔진의 형태로 동작하는 스크립트 언어
	...
	소스의 구성요소
	1. 정적 텍스트 : HTML, JS, CSS, 일반 텍스트 , foreground module 로 동작
	2. 스크립트 요소 : background module 로 동작
		- directive(지시자), &lt;%@ 지시자명 속성명="속성값"  %&gt; : 현재 JSP 페이지에 대한 설정정보 
		...
		%>
		- expression(표현식) : <%=sample %>, <%=this.sample %>, <%=testMtd() %>
		- declaration(선언부) : <%! %>
		<%!
