...
<pre> case 1~5까지! 
	...
	1. Response Line : 서버의 요청 처리 결과를 표현하는 상태 코드(response status code)
		* response status code
		...
		200~ : OK(SUCCESS)
		300~ : 최종적으로 요청 처리가 완료되려면, 클라이언트로 추가 액션이 필요함. response body 가 없음.
		...
		400~ : client side error
			...
			406/415 : 전송 컨텐츠 타입과 연관된 상태코드
				...
				415(UnSupported Media Type) : reuqest Content-Type헤더와 연관됨.
	...
	3. Response Body(Content Body, Message Body)
		...
		4) ServletOuputStream response.getOutputStream() 객체 이용
		
		===> reaponse body 데이터를 직렬화.
	<%--
		out.println("text");
